...
const postGames = async (req, res) => {
  try {
    ...
    if (!name || !description || !platforms || !image || !date || !rating || !genres) {
      ...
      //   const [foundGenre, createdGenre] = await Genres.findOrCreate({
      //     where: { genres: genre },
      //   });

      //   // Asocia el género al videojuego si no existe ya la asociación
      //   if (createdGenre) {
      //     await game.addGenres(foundGenre);
      //   }
      // }
      ...
      if (!game) {
        // Si el juego ya existía, envía un mensaje
